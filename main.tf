...
locals {
  ...
  azs_private = slice(data.aws_availability_zones.available.names, 0, 2)
}
...
resource "aws_subnet" "public" {
  ...
  availability_zone       = local.azs_public[count.index]
...
resource "aws_subnet" "private" {
  ...
  availability_zone = local.azs_private[count.index]
...
data "aws_iam_policy_document" "ec2_logging" {
  statement {
    effect = "Allow"
    actions = [
      "logs:CreateLogGroup",
...
resource "aws_iam_role_policy" "ec2_logging" {
  name   = "${var.project}-ec2-logging"
  role   = aws_iam_role.ec2.id
  policy = data.aws_iam_policy_document.ec2_logging.json
}
